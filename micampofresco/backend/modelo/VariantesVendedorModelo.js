...
export const VariantesModelo = {
  ...
  async obtenerVariantes() {
    try {
      const rows = await ejecutarConsulta('SELECT * FROM variantes');
      return rows; // ya es un array
    } catch (error) {
  ...
  async crearVariante(data) {
    try {
      const {
        ...
        peso, fecha_caducidad, ingredientes,
        estado, impuestos
      } = data;

      const rows = await ejecutarConsulta(
        `INSERT INTO variantes (
           idproducto, nombre, precio, stock, stock_minimo, stock_maximo,
           unidad_medida, peso, fecha_caducidad, ingredientes, estado,
           impuestos, created_at, updated_at
         )
         VALUES (
  ...
  async actualizarVariante(idvariante, data) {
    try {
      const {
        ...
        stock_maximo, unidad_medida, peso,
        fecha_caducidad, ingredientes, estado, impuestos
      } = data;

      const rows = await ejecutarConsulta(
        `UPDATE variantes
         SET nombre=$1, precio=$2, stock=$3, stock_minimo=$4,
             stock_maximo=$5, unidad_medida=$6, peso=$7,
             fecha_caducidad=$8, ingredientes=$9, estado=$10,
             impuestos=$11, updated_at=NOW()
         WHERE idvariante=$12
         RETURNING *`,
        [
          nombre, precio, stock, stock_minimo,
          stock_maximo, unidad_medida, peso,
          fecha_caducidad, ingredientes, estado,
          impuestos, idvariante
        ]
      );
