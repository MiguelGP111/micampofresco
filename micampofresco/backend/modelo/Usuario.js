...
class Usuario {
    ...
    static async obtenerTodos() {
        try {
            const consulta = `
                SELECT idusuario, nombre, email, rol, estado, created_at, updated_at 
                FROM usuarios 
    ...
    static async obtenerPorEmailConPassword(email) {
        try {
            const consulta = `
                SELECT idusuario, nombre, email, password, rol, estado, created_at, updated_at 
                FROM usuarios
                WHERE email = ?
            `;
            const resultados = await ejecutarConsulta(consulta, [email]);
    ...
    validar() {
        ...
        if (this.rol && !['admin', 'usuario', 'vendedor'].includes(this.rol)) {
            errores.push('El rol debe ser: admin, usuario o vendedor');
        }
